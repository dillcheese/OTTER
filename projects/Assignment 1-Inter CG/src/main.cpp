...
#pragma region Shader and ImGui
		...
		// These are our application / scene level uniforms that don't necessarily update every frame
		...
		shader->SetUniform("u_Ambient", (int)option1);
		...
		shader->SetUniform("time", WaveTime);
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::Checkbox("Ambient + Specular + Diffuse", &option4)) {
				option1 = false;
				option2 = false;
				option3 = false;
				option5 = false;
			}
			if (ImGui::Checkbox("Ambient + Specular + Flashing Effect", &option5)) {
				option1 = false;
				option2 = false;
				option3 = false;
				option4 = false;
...
// Load some textures from files
		...
		Texture2D::sptr boat = Texture2D::LoadFromFile("images/Boat 1 Texture.png");
		Texture2D::sptr dam = Texture2D::LoadFromFile("images/Dam.png");
		Texture2D::sptr gold = Texture2D::LoadFromFile("images/gold.jpg");
		Texture2D::sptr silver = Texture2D::LoadFromFile("images/silver.jpg");
		Texture2D::sptr copper = Texture2D::LoadFromFile("images/space.png");
...
// We need to tell our scene system what extra component types we want to support
		...
		material0->Set("s_Diffuse", silver);
		material0->Set("s_Diffuse2", silver);
		material0->Set("s_Specular", specular);
		material0->Set("u_Shininess", 8.0f);
		material0->Set("u_TextureMix", 0.5f);	

		ShaderMaterial::sptr materialBoat = ShaderMaterial::Create();
		materialBoat->Shader = shader;
		materialBoat->Set("s_Diffuse", boat);
		materialBoat->Set("s_Diffuse2", boat);
		materialBoat->Set("s_Specular", specular);
		materialBoat->Set("u_Shininess", 8.0f);
		materialBoat->Set("u_TextureMix", 0.5f);
		
		ShaderMaterial::sptr materialDam = ShaderMaterial::Create();
		materialDam->Shader = shader;
		materialDam->Set("s_Diffuse", dam);
		materialDam->Set("s_Diffuse2", dam);
		materialDam->Set("s_Specular", specular);
		materialDam->Set("u_Shininess", 10.0f);
		materialDam->Set("u_TextureMix", 0.5f);

		ShaderMaterial::sptr materialGold = ShaderMaterial::Create();
		materialGold->Shader = shader;
		materialGold->Set("s_Diffuse", gold);
		materialGold->Set("s_Diffuse2", gold);
		materialGold->Set("s_Specular", specular);
		materialGold->Set("u_Shininess", 10.0f);
		materialGold->Set("u_TextureMix", 0.5f);

		ShaderMaterial::sptr materialCopper = ShaderMaterial::Create();
		materialCopper->Shader = shader;
		materialCopper->Set("s_Diffuse", copper);
		materialCopper->Set("s_Diffuse2", copper);
		materialCopper->Set("s_Specular", specular);
		materialCopper->Set("u_Shininess", 10.0f);
		materialCopper->Set("u_TextureMix", 0.5f);
		...
		GameObject sceneObj = scene->CreateEntity("scene_geo"); //scene box ground
		{
			VertexArrayObject::sptr sceneVao = NotObjLoader::LoadFromFile("Sample.notobj");
			sceneObj.emplace<RendererComponent>().SetMesh(sceneVao).SetMaterial(materialGold);
			sceneObj.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.0f);
		}

		GameObject damObj = scene->CreateEntity("dam"); //dam
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Dam.obj");
			damObj.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialDam);
			damObj.get<Transform>().SetLocalScale(1.0f, 1.0f, 1.0f);
			damObj.get<Transform>().SetLocalRotation(90.0f, 0.0f, 180.0f);
			damObj.get<Transform>().SetLocalPosition(0.0f, -75.0f, -5.0f);
		}

		GameObject obj2 = scene->CreateEntity("monkey GOD");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/monkey.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialCopper);
			obj2.get<Transform>().SetLocalPosition(0.0f, -90.0f, 40.0f); // z is up or down y is forward or backward
			obj2.get<Transform>().SetLocalRotation(0.0f, 0.0f, -90.0f);
			obj2.get<Transform>().SetLocalScale(40.0f, 40.0f, 40.0f);
			//BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(obj2);
			//// Set up a path for the object to follow
			pathing->Points.push_back({ 0.0f, -90.0f, 40.1f });
			pathing->Points.push_back({ 0.0f, -90.0f, 45.0f });
			pathing->Speed = 2.0f;
			printf("HEEEEEEEEEEEEEEEEEEEEEEEEEEEE");
		}

		GameObject obj3 = scene->CreateEntity("monkey GOD 1");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/monkey.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialGold);
			obj3.get<Transform>().SetLocalPosition(75.0f, -95.0f, 22.0f); // z is up or down y is forward or backward
			obj3.get<Transform>().SetLocalRotation(0.0f, 0.0f, -90.0f);
			obj3.get<Transform>().SetLocalScale(8.0f, 8.0f, 8.0f);

			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(obj3);
			//// Set up a path for the object to follow
			pathing->Points.push_back({ 75.0f, -95.0f, 22.1f });
			pathing->Points.push_back({ 75.0f, -95.0f, 25.0f });
			pathing->Speed = 5.0f;
		}

		GameObject obj5 = scene->CreateEntity("monkey GOD 2");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/monkey.obj");
			obj5.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialGold);
			obj5.get<Transform>().SetLocalPosition(-75.0f, -95.0f, 22.0f); // z is up or down y is forward or backward
			obj5.get<Transform>().SetLocalRotation(0.0f, 0.0f, -90.0f);
			obj5.get<Transform>().SetLocalScale(8.0f, 8.0f, 8.0f);
		...
		GameObject obj4 = scene->CreateEntity("moving_boat");
		{
			// Build a mesh
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Boat 1.obj");

			obj4.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialBoat);
			obj4.get<Transform>().SetLocalPosition(-1.0f, 0.0f, 2.0f);
			obj4.get<Transform>().SetLocalScale(0.05f, 0.05f, 0.05f);
			...
			// Set up a path for the object to follow
			pathing->Points.push_back({ -4.0f, -4.0f, 0.10f });
			pathing->Points.push_back({ 4.0f, -4.0f, 0.10f });
			pathing->Points.push_back({ 4.0f,  4.0f, 0.10f });
			pathing->Points.push_back({ -4.0f,  4.0f, 0.10f });
			pathing->Speed = 2.0f;
		...
		GameObject obj6 = scene->CreateEntity("following_monkey");
		{
			...
			obj6.get<Transform>().SetLocalPosition(0.0f, 0.0f, 30.0f);
			obj6.get<Transform>().SetLocalRotation(0.0f, 0.0f, 90.0f);
			obj6.get<Transform>().SetLocalScale(10.0f, 10.0f, 10.0f);
			obj6.get<Transform>().SetParent(obj4);

			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(obj6);
			// Set up a path for the object to follow
			pathing->Points.push_back({ 0.0f, 0.0f, 10.0f });
			pathing->Points.push_back({ 0.0f, 0.0f, 25.0f });
			pathing->Speed = 10.0f;
		}
