...
bool initGLFW() {
	...
	window = glfwCreateWindow(800, 800, "100750922", nullptr, nullptr);
...
#pragma region Shader and ImGui
		...
		shader->LoadShaderPartFromFile("shaders/terrain_vertex_shader.glsl", GL_VERTEX_SHADER);
		shader->LoadShaderPartFromFile("shaders/frag_blinn_phong_terrain_textured.glsl", GL_FRAGMENT_SHADER);
		shader->Link();
		...
		glm::vec3 HeightCutoff = glm::vec3(0.35f, 0.567f, 0.764f);
		glm::vec3 InterpolateFactor = glm::vec3(0.04f, 0.08f, 0.08f);
		...
		shader->SetUniform("u_HeightCutoff", HeightCutoff);
		shader->SetUniform("u_InterpolateFactor", InterpolateFactor);
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::SliderFloat3("Height Cutoff", &HeightCutoff[0], 0, 1)) {
				shader->SetUniform("u_HeightCutoff", HeightCutoff);
			}
			if (ImGui::SliderFloat3("Interpolate Factor", &InterpolateFactor[0], 0, 1)) {
				shader->SetUniform("u_InterpolateFactor", InterpolateFactor);
			}
