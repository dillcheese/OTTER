...
void main() {
	...
	vec4 textureColor = texture(s_Third, inUV);
	vec4 textureColor2 = texture(s_Diffuse, inUV);

	//float u_TexMix = mix(textureColor, textureColor2);
	float u_TexMix = mix(textureColor.x, textureColor2.x, 0.4f);

	vec3 result = (
		(u_AmbientCol * u_AmbientStrength) + // global ambient light
		(ambient + diffuse + specular) * attenuation // light factors from our single light
		) * inColor *  u_TexMix; // Object color

	frag_color = vec4(result, textureColor.a);
} // 1:06: 20 youtube 7   textureColor.rgb